...
export default function Brands() {
  ...
  const { items: brands = [], loading } = useSelector((s) => s.brands);
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <main className="flex-1 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="bg-[#0A1F44] text-white px-4 py-2 rounded hover:opacity-90">
            + Add Brand
          </button>
        </div>
        ...
        <div className="border rounded-lg overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan="2" className="p-6 text-center text-[#6B7280]">
                    Loading brands...
                  </td>
                </tr>
              )}
              ...
              {brands.map((brand) => (
                <tr key={brand._id} className="border-t hover:bg-gray-50">
                  <td className="p-3 text-[#0A1F44] font-medium">{brand.name}</td>
